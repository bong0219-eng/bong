...
function syncCoverUpdateVersionState(){
  try{
    ...
    function setVersionButton(label, suffix){
      try{
        btn.textContent = '';
        var m = /^(V\d+)(-\d+)$/.exec(String(label || ''));
        if(m){
          var main = document.createElement('span');
          main.className = 'oai-version-main';
          main.textContent = m[1];
          var sub = document.createElement('span');
          sub.className = 'oai-version-sub';
          sub.textContent = m[2];
        ...
        btn.textContent = String(label || '') + suffix;
      }
    }
    setVersionButton(target, mismatch ? ' 업데이트 필요' : ' 새로고침');
